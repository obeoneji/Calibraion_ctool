...
td {
  ...
  void FormatMatrixToHtml(std::ostream& stream,const cv::Mat& matrix,const std::string& title) {
    stream << "<h3>" << title << ":</h3><table border='1'>" << std::endl;
    ...
    for (int i = 0; i < matrix.rows; ++i) {
      ...
      {              
        if (matrix.type() == CV_32F) {
          stream << "<td>" << matrix.at<float>(i, j) << "</td>";
        }
        else{
          stream << "<td>" << matrix.at<double>(i, j) << "</td>";          
        }
  ...
  void GenerateResultHtml(std::ostream& stream, const calibmar::Calibration& calibration) {
    if(calibration.System())
    {
      //camnum
      stream << "<p>" << "<h3>Num of cameras:</h3>" << std::endl
          << calibration.cam_num<< std::endl<< std::endl;
      ...
      { 
        ...
        switch (calibration.G2O_solver_flag()) {
            case 0: // Eigen
                // 执行与 Eigen 求解器相关的操作
                g2o_solver_name = "Eigen";
                break;
            case 1: // Cholmod
                g2o_solver_name = "Cholmod";
      ...
      for(int i=0;i<calibration.cam_num;i++)
      {
        ...
        // camera matrix
        stream << "<h3>Camera "<<i<<" Matrix:</h3>" << std::endl
        ...
        if(calibration.G2O_flag())
        {        
          const std::string  cam_string_before="Before bundle adjustment. Camera "+std::to_string(i);
          FormatMatrixToHtml(stream, calibration.InitalPose(i),cam_string_before);
          const std::string  cam_string_after="After bundle adjustment. Camera "+std::to_string(i);
