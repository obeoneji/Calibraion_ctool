...
#include "ui/utils/system_calibration_runner.h"
...
namespace calibmar {
  ...
  void MainWindow::NewFilesCalibration() {
    ...
    for (int i=0;i<file_calibration_options_.cam_num;i++)
    {
      ...
      calibrations_.push_back(std::move(calibration)); // 将新实例添加到向量中
      file_calibration_options_.current_index=i;
      // QMessageBox::information(this, "Validation Error", "initial_camera_count_!=1"
      CalibrationWidget* calibration_widget = new CalibrationWidget(
          this, std::bind(&MainWindow::DisplayExtractionImage, this, std::placeholders::_1, std::placeholders::_2));
      main_layout_->addWidget(calibration_widget);

      std::unique_ptr<FilesCalibrationRunner> runner =std::make_unique<FilesCalibrationRunner>(calibration_widget, file_calibration_options_);

      // worker_threads.push_back(std::move(worker_thread));
        // 在当前线程中运行，而不是新线程
      calibration_success_ = runner->Run(*calibrations_.back().get());
      if (calibration_success_) {
      ...
      // worker_thread_.reset(new std::thread([this, runner = std::move(runner), calibration = calibration_.get()]() {
      //   calibration_success_ = runner->Run(*calibration);QMetaObject::invokeMethod(this, [this]() { EndNewCalibration(); });
      // }));

      // worker_thread_->detach();
    }
  }
  void MainWindow::CalibrationSystem() {
    CalibrationSystemDialog dialog(this);
    dialog.SetOptions(calibration_system_options_);
    dialog.resize(500,500);
    dialog.exec();
    if (dialog.result() != QDialog::DialogCode::Accepted) {
      return;
    ...
    last_output_directory_ = calibration_system_options_.output_images_directory;
    calibration_.reset(new Calibration());
    // auto calibration = std::make_unique<Calibration>();
    // calibrations_.push_back(std::move(calibration)); // 将新实例添加到向量中
    CalibrationWidget* calibration_widget = new CalibrationWidget(
    ...
    std::unique_ptr<SystemCalibrationRunner> runner =
        std::make_unique<SystemCalibrationRunner>(calibration_widget, calibration_system_options_);
    ...
    worker_thread_->detach();
    // std::unique_ptr<SystemCalibrationRunner> runner =std::make_unique<SystemCalibrationRunner>(calibration_widget, calibration_system_options_);
    // calibration_success_ = runner->Run(*calibration_);
    // if (calibration_success_) {
    //   successful_calibrations++;
    // }
    // // 更新 UI
  ...
  void MainWindow::SaveCalibration() {
    if (calibrations_.empty()&&!calibration_) {
  ...
  void MainWindow::CreateActions() {
    ...
    calibration_system_action_ = fileMenu->addAction("&Calibrate Camera System...", this, &MainWindow::CalibrationSystem);
    fileMenu->addSeparator();
    calibration_files_action_ = fileMenu->addAction("&Calibrate Dome offset...", this, &MainWindow::NewFilesCalibration);
  ...
  void MainWindow::BeginNewCalibration() {
    ...
    calibration_system_action_->setEnabled(false);
  ...
  void MainWindow::EndNewCalibration() {
    ...
    calibration_system_action_->setEnabled(true);
