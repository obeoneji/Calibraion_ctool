...
namespace calibmar {
  ...
  bool UndistortionDialog::StartUndistortingImages() {
    ...
    if (!camera_model_edit_->Validate(msg,0)) {//liheng3
    ...
    //   return false;
    // }
    if (camera_model_edit_->InitialCameraParameters(1).empty()) {//liheng3
      QMessageBox::critical(this, "", "Camera parameters required!");
      return false;
    }
    ...
    std::unique_ptr<colmap::Camera> camera = std::make_unique<colmap::Camera>(CameraModel::InitCamera(
        camera_model_edit_->CameraModel(), {width, height}, camera_model_edit_->InitialCameraParameters(1)[0]));//liheng3
