...
namespace calibmar {
  class MultiCharucoBoardFeatureExtractor : public FeatureExtractor {
    public:
      struct Options {
      ...
      MultiCharucoBoardFeatureExtractor(const Options& options);
      ...
      const std::map<uint32_t, std::map<uint32_t, Eigen::Vector3d>>& Points3D();

    private:
      Options options_;

      std::map<uint32_t,std::map<uint32_t, Eigen::Vector3d>> points3D_map;
      cv::Ptr<cv::aruco::CharucoBoard> board_;
    };
  inline const std::map<uint32_t, std::map<uint32_t, Eigen::Vector3d>>& MultiCharucoBoardFeatureExtractor::Points3D() {
    return points3D_map;
  }
}  // namespace calibmar